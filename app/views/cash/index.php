...
<div class="transaction-page cash-page">
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3"><div class="card stat-card stat-sales"><div class="card-body"><div class="stat-label"><?= $session ? 'Session Sales' : 'Today Sales' ?></div><div class="stat-value"><?= $symbol ?> <?= money($session ? $sessionSales : $todaySales) ?></div></div></div></div>
        <div class="col-6 col-md-3"><div class="card stat-card stat-expenses"><div class="card-body"><div class="stat-label"><?= $session ? 'Session Expenses' : 'Today Expenses' ?></div><div class="stat-value"><?= $symbol ?> <?= money($session ? $sessionExpenses : $todayExpenses) ?></div></div></div></div>
        ...
        <div class="col-6 col-md-3"><div class="card stat-card stat-purchases"><div class="card-body"><div class="stat-label">Status</div><div class="stat-value"><?= $session ? 'Open' : 'Closed' ?></div></div></div></div>
    </div>

    <?php if (!empty($todaySessions)): ?>
        <div class="alert alert-light border mb-4">
            <strong>Today:</strong> <?= count($todaySessions) ?> session<?= count($todaySessions) > 1 ? 's' : '' ?>
            (<?= count(array_filter($todaySessions, fn($s) => $s['status'] === 'closed')) ?> closed)
        </div>
    <?php endif; ?>
        ...
        <div class="col-lg-6">
            <?php if (!$session): ?>
                <div class="table-card form-card cash-action-card">
                    ...
                    <p class="text-muted small mb-3">Type the hand cash amount in the drawer to start billing. You can open and close multiple times per day.</p>
                ...
                <?php
                $closedToday = array_filter($todaySessions, fn($s) => $s['status'] === 'closed');
                if (!empty($closedToday)):
                ...
                ?>
                    <div class="table-card mt-3">
                        ...
                        <div class="transaction-detail-grid">
                            ...
                            <div class="transaction-detail"><span class="transaction-detail-label">Open Cash</span><span><?= $symbol ?> <?= money($lastClosed['opening_balance']) ?></span></div>
                            <div class="transaction-detail"><span class="transaction-detail-label">Close Cash</span><span><?= $symbol ?> <?= money($lastClosed['cash_received']) ?></span></div>
                            <div class="transaction-detail"><span class="transaction-detail-label">Session Sales</span><span><?= $symbol ?> <?= money($lastClosed['total_sales']) ?></span></div>
                            <div class="transaction-detail"><span class="transaction-detail-label">Difference</span><span class="<?= (float)$lastClosed['cash_difference'] >= 0 ? 'text-profit' : 'text-loss' ?>"><?= $symbol ?> <?= money($lastClosed['cash_difference']) ?></span></div>
                        </div>
                    </div>
                <?php endif; ?>
            <?php else: ?>
                ...
                <div class="table-card form-card cash-action-card">
                    ...
                    <p class="text-muted small mb-3">Count cash in drawer and type the total amount to close this session.</p>
        ...
        <div class="col-lg-6">
            <?php if (!empty($todaySessions)): ?>
                <div class="table-card mb-3">
                    <h5 class="card-title"><i class="bi bi-calendar-day"></i> Today's Sessions</h5>
                    <?php foreach (array_reverse($todaySessions) as $h): ?>
                        <div class="stock-list-item">
                            <span>
                                <?= e(date('h:i A', strtotime($h['opened_at']))) ?>
